...
for (const filename of files) {
  ...
  for (let i = 0; i < lines.length; ++i) {
    ...
    switch (key) {
      case "startCharMetrics":
        const metrics = lines.splice(i + 1, Number.parseInt(val));

        metrics.forEach((metric) => {
          const name = metric.match(/\bN\s+(\.?\w+)\s*;/)[1];
          glyphWidths[name] = Number.parseInt(metric.match(/\bWX\s+(\d+)\s*;/)[1], 10);
        });
      ...
      case "startKernPairs":
        const pairs = lines.splice(i + 1, Number.parseInt(val));
        ...
        // KPX o comma -40
          ...
          kerning[left][right] = Number.parseFloat(values[3], 10);
        }
      ...
      case "underlineThickness":
        properties[key] = Number.parseFloat(val, 10);
        break;
  ...
  const widths = Array.from({ length: 256 });
  for (let i = 0; i < 256; ++i) {
